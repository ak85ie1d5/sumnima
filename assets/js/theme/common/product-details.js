...
export default class ProductDetails extends ProductDetailsBase {
    constructor($scope, context, productAttributesData = {}) {
        ...
        if($(".modal .quickView").length == 0) {
            this.loadSwiperSlider();
        }

        //get active currency code
        let bearerToken = context.bearerToken;
        let currencyCode = context.currencyCode;
        this.getActiveCurrencyCode(bearerToken, currencyCode);
        ...
        $productOptionsElement.on('change', event => {
            this.productOptionsChanged(event, context);
    ...
    getActiveCurrencyCode(bearerToken, currencyCode){
        var query =
        ...
        var graphql_query_result = $.ajax({
            ...
            success: function(productData) {
                $("#currency_code").val(productData.data.site.currency.display.symbol);
            }
        });
    }
    ...
    loadSwiperSlider() {
        ...
        const productSwiper = new Swiper(".productView:not(.productView--quickView) .productView-image", {
        // slidesPerView: "auto",
            initialSlide: 0,
            slidesPerView: 1,
            slidesPerGroup: 1,
        ...
        $(".productView:not(.productView--quickView) .productView-thumbnails .productView-thumbnail:nth-child(1) .productView-thumbnail-link").addClass("is-active");
        
        $(window).on('resize', function(){
            $(".productView:not(.productView--quickView) .productView-images figure.productView-image").css("height", "");
        });

        productSwiper.on('slideChange', function (swiper) {
            //////console.log(swiper);
            let activeIndex = swiper.activeIndex;
            $(".productView-thumbnails .productView-thumbnail .productView-thumbnail-link").removeClass("is-active");
            $(".productView-thumbnails .productView-thumbnail:nth-child("+ (activeIndex + 1) +") .productView-thumbnail-link").addClass("is-active");
            ////console.log('productSwiper slide changed');
        });

        $(".productView:not(.productView--quickView) .productView-thumbnails .productView-thumbnail").on('mouseover click touchStart', function (event) {
            let clickedIndex = $(this).index();
            productSwiper.slideTo(clickedIndex, false);
            $(".productView-thumbnails .productView-thumbnail .productView-thumbnail-link").removeClass("is-active");
            $(this).find(".productView-thumbnail-link").addClass("is-active");
            ////console.log('productThumbSwiper click changed');
        });
    }

    filterProductImages(colorOption, context){
        let that = this;

        ////console.log(context);

        let productImages = context.productImages;
        let default_image_product = context.loaderImage;
        let zoom_size = context.zoom_size;
        let product_size = context.product_size;
        let blur_product_main_size = context.blur_product_main_size;
        let blur_product_thumb_size = context.blur_product_thumb_size;
        let productview_thumb_size = context.productview_thumb_size;
        let productImagesHTML = '';
        let productMainImagesHTML = '';
        let productAllImagesHTML = '';
        let productAllMainImagesHTML = '';
        
        let productHeight = $(".product .productView-image .productView-img-container").outerHeight(true);
        //console.log(productHeight);
        $(".productView .productView-images figure.productView-image").css("height", productHeight + "px");
        productImages.forEach(function(image, index){
            let imageAlt = image.alt;
            let imageUrl = image.data ? image.data : default_image_product;
            let imageUrl_zoom_size = imageUrl.replace("{:size}", zoom_size);
            let imageUrl_product_size = imageUrl.replace("{:size}", product_size);
            let imageUrl_product_blur_size = imageUrl.replace("{:size}", blur_product_main_size);
            let imageUrl_productview_blur_thumb_size = imageUrl.replace("{:size}", blur_product_thumb_size);
            let imageUrl_productview_thumb_size = imageUrl.replace("{:size}", productview_thumb_size);
            if(imageAlt != "" && (imageAlt.toLowerCase().indexOf(colorOption.toLowerCase()) > -1)) {
                productImagesHTML += `<li class="productView-thumbnail">
                                        <div class="productView-thumbnail-link">
                                            <img src="${imageUrl_product_blur_size}" alt="${imageAlt}" title="${imageAlt}" data-sizes="auto" data-src="${imageUrl_product_size}" class="lazyload" />
                                        </div>    
                                    </li>`;

                productMainImagesHTML += `<img src="${imageUrl_product_blur_size}" alt="${imageAlt}" title="${imageAlt}" data-sizes="auto" data-src="${imageUrl_product_size}" class="lazyload swiper-slide" />`;
            }

            productAllImagesHTML += `<li class="productView-thumbnail">
                                        <div
                                            class="productView-thumbnail-link"
                                        >
                                        <img src="${imageUrl_productview_blur_thumb_size}" data-src="${imageUrl_productview_thumb_size}" alt="${imageAlt}" title="${imageAlt}" data-sizes="auto" class="lazyload">
                                    </div>
                                </li>`;

            productAllMainImagesHTML += `<img src="${imageUrl_product_blur_size}" alt="${imageAlt}" title="${imageAlt}" data-sizes="auto" data-src="${imageUrl_product_size}" class="lazyload swiper-slide" />`;

            if((index + 1) == productImages.length) {
                
                if(productImagesHTML  != '') {
                    $(".productView-thumbnails").html(productImagesHTML);
                    $(".productView-image .productView-img-container").html(productMainImagesHTML);
                } else {
                    $(".productView-thumbnails").html(productAllImagesHTML);
                    $(".productView-image .productView-img-container").html(productAllMainImagesHTML);
                }


                that.loadSwiperSlider();
                
                //that.loadSwiperInProductThumb();
            }
        }); 
    }

    quickViewFilterProductImages(colorOption, context){
        var that = this;

        //console.log(colorOption);
        ////console.log(context);

        let productImageString = $(".quickView .productImages").val();

        let productImages = JSON.parse(productImageString);
        let default_image_product = $(".quickView .loaderImage").val();
        let zoom_size = $(".quickView .zoom_size").val();
        let product_size = $(".quickView .product_size").val();
        let blur_product_main_size = $(".quickView .blur_product_main_size").val();
        let blur_product_thumb_size = $(".quickView .blur_product_thumb_size").val();
        let productview_thumb_size = $(".quickView .productview_thumb_size").val();
        let productImagesHTML = '';
        let productMainImagesHTML = '';
        let productAllImagesHTML = '';
        let productAllMainImagesHTML = '';
        productImages.forEach(function(image, index){
            ...
            if(imageAlt != "" && (imageAlt.toLowerCase().indexOf(colorOption.toLowerCase()) > -1)) {
                productImagesHTML += `<li class="productView-thumbnail">
                                        <div class="productView-thumbnail-link">
                                            <img src="${imageUrl_product_blur_size}" alt="${imageAlt}" title="${imageAlt}" data-sizes="auto" data-src="${imageUrl_product_size}" class="lazyload swiper-slide" />
                                        </div>    
                                    </li>`;

                productMainImagesHTML += `<img src="${imageUrl_product_blur_size}" alt="${imageAlt}" title="${imageAlt}" data-sizes="auto" data-src="${imageUrl_product_size}" class="lazyload" />`;
            }
            ...
            productAllImagesHTML += `<li class="productView-thumbnail">
                                        <div
                                            class="productView-thumbnail-link"
                                        >
                                        <img src="${imageUrl_productview_blur_thumb_size}" data-src="${imageUrl_productview_thumb_size}" alt="${imageAlt}" title="${imageAlt}" data-sizes="auto" class="lazyload swiper-slide">
                                    </div>
                                </li>`;

            productAllMainImagesHTML += `<img src="${imageUrl_product_blur_size}" alt="${imageAlt}" title="${imageAlt}" data-sizes="auto" data-src="${imageUrl_product_size}" class="lazyload" />`;

            if((index + 1) == productImages.length) {

                if(productImagesHTML  != '') {
                    $(".quickView .productView-thumbnails").html(productImagesHTML);
                    $(".quickView .productView-image .productView-img-container").html(productMainImagesHTML);
                } else {
                    $(".quickView .productView-thumbnails").html(productAllImagesHTML);
                    $(".quickView .productView-image .productView-img-container").html(productAllMainImagesHTML);
                }
     ...
     *
     */
    productOptionsChanged(event, context) {
        const $changedOption = $(event.target);
        const $form = $changedOption.parents('form');
        const productId = $('[name="product_id"]', $form).val();
        const optionName = $changedOption.attr("option-name");
        //console.log(optionName);


        if(optionName && optionName == "Color") {
            let colorOption = $changedOption.attr("aria-label");

            //console.log($('.modal .quickView').length);

            if($('.modal .quickView').length) {
                this.quickViewFilterProductImages(colorOption, context);
            } else {
                this.filterProductImages(colorOption, context);
            }
        }
